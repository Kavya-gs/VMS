...
const ApprovalsPage = () => {
  ...
  const rejectVisitor = async (id) => {
    try {
      ...
      console.error(error);
      const errorMessage =
        error.response?.data?.message || "Failed to reject visitor";
      toast.error(errorMessage);
    }
  };
              ...
              visitors.map((visitor) => (
                <tr key={visitor._id} className="border-t">
                  <td className="p-3">
                    {visitor.photo ? (
                      <button
                        onClick={() => setSelectedImage(visitor.photo)}
        ...
        <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl p-4 max-w-lg w-full relative">
            <button
              ...
              className="absolute top-0 right-1 text-xl font-bold text-slate-600"
            >
              ×
            </button>
